...
`ep_author_follow` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_author_follow`:
...
* Server: `clientVars`, `eejsBlock_mySettings`
* Client: `postAceInit`, `handleClientMessage_NEW_CHANGES`, `handleClientMessage_ACCEPT_COMMIT`
